...
void CardGame::setPlayers(int minPlayers, int maxPlayers)
{
	while(true)
	{
		...
		{
			if(numberOfPlayers >= minPlayers && maxPlayers == -1) // no max player
				break;
			if(minPlayers == maxPlayers) // constante player
				std::cout << "Error : Must be " << maxPlayers << " players." << std::endl;
			else if(maxPlayers == -1)
				std::cout << "Error : Must be between minimum " << minPlayers << " players." << std::endl;
			else
				std::cout << "Error : Must be between " << minPlayers << " and " << maxPlayers << " players." << std::endl;
		}
		else
			break;
	}
